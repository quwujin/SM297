...
; !function (win) {
    ...
    win.laydate = function (options) {
        ...
        if (options.hasOwnProperty("disable")) {
            var disopt = options.disable;
            for (var i = 0; i < options.disable.length; i++) {
                var objopt = (options.disable[i].replace("-", ","))
                while (objopt.indexOf("-") >= 0) {
                    objopt = (objopt.replace("-", ","))
                }
                if (objopt.split(",").length > 2) {
                    disopt[i] = parseInt(objopt.split(",")[0]) + "," + parseInt(objopt.split(",")[1]) + "," + parseInt(objopt.split(",")[2]);
                }
            }
            config.disable = disopt;
        }
        if (options.hasOwnProperty("path")) {
            config.path = options.path;
        }
        if (options.hasOwnProperty("defSkin")) {
            config.defSkin = options.defSkin;
        }
        if (options.hasOwnProperty("format")) {
            config.format = options.format;
        }
        if (options.hasOwnProperty("min")) {
            config.min = options.min;
        }
        if (options.hasOwnProperty("max")) {
            config.max = options.max;
        }
        if (options.hasOwnProperty("isweek")) {
            config.isweek = options.isweek;
        }
		if (options.hasOwnProperty("Interval")) { //时间区间 2016-04-18,2016-04-21;2016-05-24,2016-05-25
            config.Interval = options.Interval;
			
        }
        if (options.hasOwnProperty("isv")) {
            config.isv = options.isv;
        }
        try {
    ...
    //检测是否在为周末
    Dates.checkWeekendVoid = function (YY, MM, DD) {
        var back = [];
        YY = YY | 0;
        MM = MM | 0;
        DD = DD | 0;
	...
	//检测是否在有效期区间内
    Dates.checkQjVoid = function (YY, MM, DD) {
        var back = [];
        YY = YY | 0;
        MM = MM | 0;
        DD = DD | 0;
		for(var i=0;i<Dates.Intervalmm.length;i++){
			var erval=Dates.Intervalmm[i];
			var Intervalmins=erval.split(',')[0].match(/\d+/g);
			var Intervalmaxs=erval.split(',')[1].match(/\d+/g);
			
			if(YY <= Intervalmaxs[0] && YY>= Intervalmins[0]){ 
				if(MM == Intervalmins[1]){ 
					if(MM == Intervalmaxs[1]){
						if(DD >= Intervalmins[2] && DD <= Intervalmaxs[2]){
							back = ['d'];
						}
					}else if(DD >= Intervalmins[2]){ 
							back = ['d'];
					}  
				}
				if(MM <= Intervalmaxs[1] && MM > Intervalmins[1]){
					if(MM <Intervalmaxs[1]){  
							back = ['d']; 
					}else if(MM == Intervalmaxs[1]){  
						if(DD <= Intervalmaxs[2]){ 
							back = ['d'];
						}	
					}	
    ...
    Dates.viewDate = function (Y, M, D) {
        ...
        Dates.each(as.tds, function (i, elem) {
			...
			//检查日期是否在限制区间内
			if (Dates.checkQjVoid(YY, MM, DD)[0]) {
                Dates.addClass(elem, as[1]);
			}
			console.log(config.isweek);

			if (config.isweek == "True") { 
			    //检查日期是否在周末
			    if (Dates.checkWeekendVoid(YY, MM, DD)[0]) {
			        Dates.addClass(elem, as[1]);
			    }
			}
            ...
            for (var i = 0; i < config.disable.length; i++) {
                var cdis = config.disable[i].split(",");
                if (cdis.length > 2) {
                    if (parseInt(cdis[0]) == YY && parseInt(cdis[1]) == MM && parseInt(cdis[2]) == DD) {
                        elem.className = "laydate_void";
                    }
                }
            }
            YY = MM = DD = null;
    ...
    Dates.iswrite = function () {
        ...
        Dates.shde(log.time, !Dates.options.istime);//隐藏选择时间按钮
		...
		Dates.shde(as.otoday, true);//隐藏今天按钮
    ...
    Dates.view = function (elem, options) {
		...
		Dates.Intervalmm = log.Intervalmm = [];
		 if (Dates.options.hasOwnProperty("Interval")) {
			 if(Dates.options.Interval.length>0){ 
				for(var i=0;i<Dates.options.Interval.split(';').length;i++){
					log.Intervalmm[i]=Dates.options.Interval.split(';')[i];
				}
			}
		}
       /* Dates.Intervalmins = log.Intervalmm[0].match(/\d+/g);
        Dates.Intervalmaxs = log.Intervalmm[1].match(/\d+/g);*/
        ...
        if (!Dates.box) {
            ...
            + '<div class="laydate_bottom">'
              ...
              + '</div>'
              + (config.isv ? '<a href="" class="laydate_v" target="_blank">laydate-v' + laydate.v + '</a>' : '')
            + '</div>';
