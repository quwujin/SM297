	...
	photoGallery: function(options) {
        ...
        //滚轮滚动
	  	$(window).on("resize",function(){
	  	    ...
	  	    var _delta = parseInt(e.originalEvent.wheelDelta);
	  	    event.returnValue = false;//禁止页面滚轮滚动  PS：不支持火狐
        	...
        	//向下滚动
	  	...
	  	}).on("DOMMouseScroll", function (e) { //火狐滚轮事件 
	  	    var _delta = parseInt(-e.originalEvent.detail);
	  	    //向上滚动
	  	    if (_delta > 0) {
	  	        biggerImage();
	  	    }
	  	    ...
	  	    e.returnValue = false; //禁止页面滚轮滚动  PS：仅支持火狐 
...
$.extend({
    ...
    openPhotoGallery: function (obj) {
	    ...
	    //var $gallerys = $(obj).closest(".gallerys"),    //在.gallerys名称下img的class名称为.gallery-pic的集合（如果要切换图片）
		//	activeIndex=0,
        //	imgs = [];

        var activeIndex = 0, imgs = [];
	...
	initGallery : function(){
		...
		$(".closeWin").click(function () {
		    parent.document.getElementById("J_pg").style.display = "none";//解决猎豹无法关闭方法
