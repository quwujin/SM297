...
function SelectDate(obj) {
	SelectDate1(70,5,obj,0,0);
}
function SelectDate2(st,obj) {
    SelectDate1(st, 5, obj, 0, 0);
}

var a = '<div id="ContainerPanel" style="display:none;"><div id="calendarPanel" style="position: absolute; display: none;z-index: 9999;';
a = a
		+ 'background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;"></div>';
if (document.all) {
    a = '<div id="ContainerPanel" style="display:none;"><div id="calendarPanel" style="position: absolute;display: none;z-index: 9999;';
    a = a
			+ 'background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;"></div>';
    a = a
			+ '<iframe style="margin-left: 0px;margin-top: 0px;position:absolute;z-index:2000;width:expression(this.previousSibling.offsetWidth);';
    a = a + 'height:expression(this.previousSibling.offsetHeight);';
    a = a
...
a = a + '</div>';
function SelectDate1(st, en, obj, left, top) {
    $(a).appendTo($(obj).parent());
    a = "";
  //  alert($(obj).parent().html().length);
    left = document.getElementById(obj.id).offsetLeft;

		var date = new Date();
	var by = date.getFullYear() - st; // 最小值 → 10 年前
	var ey = date.getFullYear() + en; // 最大值 → 10 年后
	cal = (cal == null) ? new Calendar(by, ey, 0) : cal; // 初始化为中文版，1为英文版
	// cal.dateFormatStyle = strFormat; // 默认显示格式为:yyyy-MM-dd ,还可显示 yyyy/MM/dd
	/*
	 * var id=obj.id; var offset = $('#'+id).offset(); left = offset.left; top =
	 * offset.top; if(document.all){left=left-2;top=top-2;}
	 * $('#calendarPanel').css({ "top" : top + "px", "left" : left + "px" });
	 */
	cal.show(obj);
	/*var offset = $(obj).offset();
	offset2 = $("#mytaobao-menu").offset();
	var l = offset.left - offset2.left-1;
	var t = offset.top - 74;
	if (document.all) {
		l = l+1;
		t = t - 16;
	...
	document.getElementById("calendarPanel").style.margin = top + "px 0px 0px " + left + "px";
    //$(this).prev().show();
	/*
	 * if(document.all){ left+=-522; top+=-3; }
	 * document.getElementById("calendarPanel").style.margin=top+"px 0px 0px
	 * "+left+"px";
	 */
	/* margin-left: 85px;margin-top: 125px; */
}
/**//* 返回日期 */
...
Calendar.prototype.draw = function() {
	...
	mvAry[mvAry.length] = '      <tr>';
	for (var i = 0; i < 7; i++) {
		mvAry[mvAry.length] = '      <th align="center" style="font-weight:normal;text-align:center;background-color:'
